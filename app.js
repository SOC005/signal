...
async function lookupDomain(domain) {
  ...
  return {
    ...
    details: [
      ['Domain', domain], ['Registry handle', rdap.handle || 'Unavailable'], ['Status', status],
      ...
      ['DNSSEC delegation', clean(rdap.secureDNS?.delegation)], ['DNS sources available', `${records.filter((result) => result.status === 'fulfilled').length}/5 record types responded`],
      ['Sources', `${sources.dns}; ${sources.rdap}`],
    ],
    dns,
  };
}
